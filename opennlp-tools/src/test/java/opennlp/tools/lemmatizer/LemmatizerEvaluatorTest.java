...
public class LemmatizerEvaluatorTest {
	...
	public void testEvaluator() throws IOException {
		...
		DummyLemmaSampleStream predictedSample = new DummyLemmaSampleStream(new PlainTextByLineStream(
				MockInputStreamFactory.mockInputStreamFactory2(new File(inPredicted)), encoding), true);

		DummyLemmaSampleStream expectedSample = new DummyLemmaSampleStream(new PlainTextByLineStream(
				MockInputStreamFactory.mockInputStreamFactory2(new File(inExpected)), encoding), false);

		Lemmatizer dummyLemmatizer = DummyLemmatizer.mockLemmatizer1(predictedSample);
