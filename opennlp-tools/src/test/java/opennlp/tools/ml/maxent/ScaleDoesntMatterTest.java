...
public class ScaleDoesntMatterTest {
	 ...
	 * Strangely, if we use (1,2) and (10,20) there's no difference. If we use
	 * (0.1,0.2) and (10,20) there is a difference.
	 */
	@Test
	public void testScaleResults() throws Exception, IOException, IOException {
		...
		ObjectStream<Event> smallEventStream = new RealBasicEventStream(new PlainTextByLineStream(
				MockInputStreamFactory.mockInputStreamFactory1(smallValues), StandardCharsets.UTF_8));

		testDataIndexer.index(smallEventStream);

		EventTrainer smallModelTrainer = TrainerFactory.getEventTrainer(ModelUtil.createDefaultTrainingParameters(),
				null);
		...
		ObjectStream<Event> largeEventStream = new RealBasicEventStream(new PlainTextByLineStream(
				MockInputStreamFactory.mockInputStreamFactory1(largeValues), StandardCharsets.UTF_8));
		...
		for (int i = 0; i < smallResults.length; i++) {
			System.out.println(
					String.format("classifiy with smallModel: %1$s = %2$f", smallModel.getOutcome(i), smallResults[i]));
			System.out.println(
					String.format("classifiy with largeModel: %1$s = %2$f", largeModel.getOutcome(i), largeResults[i]));
