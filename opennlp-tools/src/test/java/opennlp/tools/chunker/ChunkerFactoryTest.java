...
public class ChunkerFactoryTest {
	...
	private static ObjectStream<ChunkSample> createSampleStream() throws IOException {
		InputStreamFactory in = ResourceAsStreamFactory.mockInputStreamFactory1(ChunkerFactoryTest.class,
				"/opennlp/tools/chunker/test.txt");

		return new ChunkSampleStream(new PlainTextByLineStream(in, StandardCharsets.UTF_8));
	}

	private static ChunkerModel trainModel(ModelType type, ChunkerFactory factory) throws IOException {
		return ChunkerME.train("eng", createSampleStream(), TrainingParameters.defaultParams(), factory);
	}
	...
	public void testDummyFactory() throws IOException {
		...
		String[] toks1 = { "Rockwell", "said", "the", "agreement", "calls", "for", "it", "to", "supply", "200",
				"additional", "so-called", "shipsets", "for", "the", "planes", "." };

		String[] tags1 = { "NNP", "VBD", "DT", "NN", "VBZ", "IN", "PRP", "TO", "VB", "CD", "JJ", "JJ", "NNS", "IN",
				"DT", "NNS", "." };
