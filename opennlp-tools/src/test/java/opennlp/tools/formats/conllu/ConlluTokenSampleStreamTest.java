...
public class ConlluTokenSampleStreamTest {
	...
	public void testParseTwoSentences() throws IOException {
		...
		try (ObjectStream<TokenSample> stream = new ConlluTokenSampleStream(new ConlluStream(streamFactory))) {
			...
			TokenSample expected1 = TokenSample.parse("Fachlich kompetent" + TokenSample.DEFAULT_SEPARATOR_CHARS
			...
			TokenSample expected2 = TokenSample.parse("Beiden Zahnärzten verdanke ich einen "
					+ "neuen Biss und dadurch endlich keine Rückenschmerzen mehr" + TokenSample.DEFAULT_SEPARATOR_CHARS
					+ ".", TokenSample.DEFAULT_SEPARATOR_CHARS);
	...
	public void testParseContraction() throws IOException {
		...
		try (ObjectStream<TokenSample> stream = new ConlluTokenSampleStream(new ConlluStream(streamFactory))) {
			...
			TokenSample expected1 = TokenSample
					.parse("Numa reunião entre representantes da Secretaria da Criança do DF "
	...
	public void testParseSpanishS300() throws IOException {
		...
		try (ObjectStream<TokenSample> stream = new ConlluTokenSampleStream(new ConlluStream(streamFactory))) {
			...
			TokenSample expected1 = TokenSample.parse("Digámoslo claramente" + TokenSample.DEFAULT_SEPARATOR_CHARS
					...
					+ TokenSample.DEFAULT_SEPARATOR_CHARS + "Gabino" + TokenSample.DEFAULT_SEPARATOR_CHARS
					+ "\" a Piedad Córdoba" + TokenSample.DEFAULT_SEPARATOR_CHARS
					+ ", en la cual no se plantea ni siquiera \"" + TokenSample.DEFAULT_SEPARATOR_CHARS
					+ "esperar un mejor gobierno" + TokenSample.DEFAULT_SEPARATOR_CHARS + "\""
					+ TokenSample.DEFAULT_SEPARATOR_CHARS + "."
