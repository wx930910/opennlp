...
public class NameSampleDataStreamTest {
	...
	public void testWithoutNameTypes() throws Exception {
		InputStreamFactory in = ResourceAsStreamFactory.mockInputStreamFactory1(getClass(),
				"/opennlp/tools/namefind/AnnotatedSentences.txt");

		NameSampleDataStream ds = new NameSampleDataStream(new PlainTextByLineStream(in, StandardCharsets.ISO_8859_1));

		NameSample ns = ds.read();

		String[] expectedNames = { "Alan McKennedy", "Julie", "Marie Clara", "Stefanie Schmidt", "Mike",
				"Stefanie Schmidt", "George", "Luise", "George Bauer", "Alisa Fernandes", "Alisa", "Mike Sander",
				"Stefan Miller", "Stefan Miller", "Stefan Miller", "Elenor Meier", "Gina Schneider", "Bruno Schulz",
				"Michel Seile", "George Miller", "Miller", "Peter Schubert", "Natalie" };
	...
	/**
	 * Checks that invalid spans cause an {@link ObjectStreamException} to be
	 * thrown.
	 */
	@Test
	...
	/**
	 * Create a NameSampleDataStream from a corpus with entities annotated with
	 * multiple nameTypes, like person, date, location and organization, and
	 * validate it.
	 *
	 * @throws Exception
	 */
	...
	public void testWithNameTypes() throws Exception {
		...
		NameSampleDataStream ds = new NameSampleDataStream(new PlainTextByLineStream(in, StandardCharsets.UTF_8));
		...
		String[] expectedPerson = { "Barack Obama", "Obama", "Obama", "Lee Myung - bak", "Obama", "Obama",
				"Scott Snyder", "Snyder", "Obama", "Obama", "Obama", "Tim Peters", "Obama", "Peters" };

		String[] expectedDate = { "Wednesday", "Thursday", "Wednesday" };

		String[] expectedLocation = { "U . S .", "South Korea", "North Korea", "China", "South Korea", "North Korea",
				"North Korea", "U . S .", "South Korea", "United States", "Pyongyang", "North Korea", "South Korea",
				"Afghanistan", "Seoul", "U . S .", "China" };
	...
	@Test
	public void testClearAdaptiveData() throws IOException, IOException {
		String trainingData = "a\n" + "b\n" + "c\n" + "\n" + "d\n";

		ObjectStream<String> untokenizedLineStream = new PlainTextByLineStream(
				MockInputStreamFactory.mockInputStreamFactory1(trainingData), StandardCharsets.UTF_8);
